...
class BookmarkTests: XCTestCase {
    ...
    override func setUp() {
        ...
        bvc = UIStoryboard(name: "Main", bundle: nil).instantiateViewControllerWithIdentifier("BookmarkViewController") as! BookmarkViewController
    ...
    func testDownloadMarkedMovie() {
        let URL = NSURL(string: "http://api.themoviedb.org/3/movie/246655?api_key=dfa910cc8fcf72c0ac1c5e26cf6f6df4")!
    ...
    func testParseMarkedMovie() {
        let filePath = NSBundle.mainBundle().pathForResource("bookmark_response",ofType:"json")
    ...
    func testMovieToPassIsPassedOnMovieDetailSegue() {
        bvc.currentMovie = Movie(id: 1, title: "title", poster: "poster", overview: "overview", popularity: 1, rate: 1, date: NSDate(), count: 1, backdrop: "backdrop")
        let segue = UIStoryboardSegue(identifier: "B_ViewMovieSegue",
                                      source: bvc,
                                      destination: mvc)
        
        bvc.prepareForSegue(segue, sender: nil)
